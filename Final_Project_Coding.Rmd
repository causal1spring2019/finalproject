...
!(DAG Causal Final Project_all covariates.png)
...
## Final DAG 
!(DAG Causal Final Project_reduced covariates.png)
...
We assume that the observed data O = (W, A, Y) ~ P_0 were generated by sampling n times from a data generating process described by the SCM. The statistical model M for the set of allowed distributions for the observed data is non-parametric. 


## Table 1

Covariate | Drug use (\%) |  No drug use (\%)
 | `r druguse[2]` (`r druguse[2]/N`\%) | `r druguse[1]` (`r druguse[1]/N`\%)
--------- | ---------- | --------- 
...
Lives with both biological parents | `r livebioparents.table[2,2]` (`r round(livebioparents.table[2,2]/N*100, 1)`\%) | `r livebioparents.table[1,2]` (`r round(livebioparents.table[1,2]/N*100, 1)`\%)
Doesn't live with both biological parents | `r livebioparents.table[2,1]` (`r round(livebioparents.table[2,1]/N*100, 1)`\%) | `r livebioparents.table[1,1]` (`r round(livebioparents.table[1,1]/N*100, 1)`\%)
$\textbf{BMI percentile}$ | `r round(mean(final_data$bmiz),1)` (mean) |  `r round(sd(final_data$bmiz),1)` (standard deviation)
...
!(DAG Causal Final Project_reduced covariates with Us.png)
...
!(DAG Causal Final Project_reduced covariates with Us_reality.png)
...
!(DAG Causal Final Project_reduced covariates with Us_convenience.png)
...
!(DAG Causal Final Project_reduced covariates with Us_final.png)
...
$$
\begin{aligned}
&\psi(\mathbb{P}_0) = \mathbb{E}_0[\mathbb{E}_0(Y|A = 1, W) - \mathbb{E}_0(Y|A = 0, W)] = \\ &\sum\limits_{w1,w2,w3,w4,w5}\bar{Q}_0(1,  W1 = w1, W2 = w2, W3 = w3, W4 = w4, W5 = w5) - \bar{Q}_0(0, W1 = w1, W2 = w2, W3 = w3, W4 = w4, W5 = w5)]\mathbb{P}_0(W1 = w1, W2 = w2, W3 = w3, W4 = w4, W5 = w5)
\end{aligned}
$$
...
Our G-computation estimator had the lowest ATE estimate of our three estimators at 0.039 (95% CI: XXXX - XXXX). Our stabilized IPTW estimator had the highest ATE estimate, at 0.045 (95% CI: XXXX - XXXX). Our TMLE estimator produced an estimate of 0.044 (95% CI 0.007 - 0.08). ****Add info about precision once the other CIs are available. Then describe the differences (or lack thereof) in the estimates provided by the different estimators.****
