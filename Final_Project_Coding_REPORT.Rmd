...
We are making exclusion restrictions, most notably that race ($W_3$) and sex ($W_2$) have no endogenous parents.
...
In our sample, approximately 19.5% of youth had been bullied before age 12, and 17% had incident drug use during follow-up. 
...
Simple Substitution: 
$\hat\Psi(P_n) = 1/n\sum_{i=1}^n(\hat{\overline{Q}(1,W_i)-\hat{\overlineQ}(0,W_i)$

IPTW with stabilized weights:
$\hat\Psi(P_n) = \frac{1/n\sum_{i=1}^{n}\frac{I(A_i=1)}{\hat{g}(A_i|W_i)}Y_i}{1/n\sum_{i=1}^{n}\frac{I(A_i=1)}{\hat{g}(A_i|W_i)}}-\frac{1/n\sum_{i=1}^{n}\frac{I(A_i=0)}{\hat{g}(A_i|W_i)}Y_i}{1/n\sum_{i=1}^{n}\frac{I(A_i=0)}{\hat{g}(A_i|W_i)}}$

TMLE: 
$\hat\Psi(P_n) = 1/n\sum_{i=1}^n(\overline{Q^*_n}(1,Wi)-\overline{Q^*_n}(0,Wi))$
