...
\vspace{12pt}
For our final project, we have worked to answer an important causal question using a real-life dataset, and the steps of the causal roadmap:
...
These data are part of a nationally representative sample; therefore, they have survey weights associated with them and are not truly a random sample of observed data $O$. However, for this project we have used a  simple link. Therefore, we assume that the observed data $O = (W, A, Y)$ ~ $\mathbb{P}_0$ were generated by sampling 7703 i.i.d. times from a data generating process described by the SCM.  

Given our knowledge of our data, the statistical model $\mathcal{M}$ for the set of allowed distributions for the observed data is non-parametric. 
...
```{r, include = TRUE, echo = FALSE, fig.width= 12, fig.height = 6}
...
$$\Large\Psi(P_0) = \frac{1/n\sum_{i=1}^{n}\frac{I(A_i=1)}{g(A_i|W_i)}Y_i}{1/n\sum_{i=1}^{n}\frac{I(A_i=1)}{g(A_i|W_i)}}-\frac{1/n\sum_{i=1}^{n}\frac{I(A_i=0)}{g(A_i|W_i)}Y_i}{1/n\sum_{i=1}^{n}\frac{I(A_i=0)}{g(A_i|W_i)}}$$
...
```{r, include = TRUE, echo = FALSE, warning = FALSE, fig.width= 16, fig.height = 11}
BootstrappedHist
```
...
Third, the independence assumptions that we created out of necessity to identify the target causal parameter were likely not correct, introducing confounder-based biased into our results.  
  
And fourth, our use of a simplified link between our observed data and our causal model (as described in step 4 of the causal roadmap, above) likely introduced some bias into our results; a more complex data link would have been ideal.
...
* \textbf{Suggestion of a dataset and potential issues for exploration:} Veronica

* \textbf{Particular expertise that we each contributed:}
    + Shelley - Project management
    + Stephanie - Pediatrics 
    + Veronica - Social and Substance Use Epi
    + Lizzy - Social and Substance Use Epi

* \textbf{Establishment of the causal model, causal question and estimand choice:} Entire group

* \textbf{Identifiability considerations:} Entire group, with Lizzy and Shelley working on the DAGs

* \textbf{Data cleaning and multiple imputation of missing covariate data:} Veronica

* \textbf{Creation of SuperLearner library:} Veronica

* \textbf{Coding of ATE point estimates:} Veronica

* \textbf{Coding of Practical Positivity Checks and Bootstrapping of Confidence Intervals:} Stephanie

* \textbf{Interpretation of Results:} Entire Group
