...
```{r DAG Initial, echo=FALSE, include = TRUE, fig.align = 'center', out.width = '60%'}
...
```{r DAG Final, echo=FALSE, include = TRUE, fig.align = 'center', out.width = '60%'}
...
#### _IPTW with stabilized weights_:
...
According to our analysis, the difference between the average counterfactual risk of drug use if everyone was bullied versus if no one was bullied is 0.04; this is the causal interpretation. We do not think our results should be interpreted causally, however, becuase of the unrealistic convenience assumptions this interpretaion requires. The statistical interpretation of our findings is that being bulled before at 12 is associated with an increase of 0.04 in the probability of subsequent drug use, compared to not being bullied. This is a plausible finding, though we expected it to be even higher. However, because drug use is typically underreported due to social desirability bias, we expect our risk difference is biased toward the null. 
...
Our analysis includes a number of limitations. First, we would have liked to have measured and included a number of covariates that were exogenous in our model, had we been prospectively collecting data. As one important example, we did not have any information on parent drug use, which we would have wanted to control for as an important confounder in our model (for identifiability we also needed to assume no shared unknowns between bullying and drug use, where this would have also fit). It is likely incorporated into our model as one of the shared unknowns of not living with both biological parents, and our outcome (drug use).  
...
Third, the independence assumptions that we created out of necessity to identify the target causal parameter were likely not accurate, introducing confounder-based bias into our results. And finally, this was not a random sample, yet our link was simplified for this project, as described in step 4, above. In subsequent analyses we would plan to use the sample weights such that these analyses would be reflective of the US population, as intended when the study was designed.
...
\item Suggestion of a dataset and potential issues for exploration: Veronica
\item Project management: Shelley
\item Background and literature review: Lizzy
\item Delineation of the causal model, causal question and estimand choice: Entire group
\item Development of the structural equations: Lizzy
\item Identifiability considerations: Shelley
\item Data cleaning and multiple imputation of missing covariate data: Veronica
\item Practical positivity checks: Stephanie
\item Creation of SuperLearner library: Veronica
\item Coding of ATE point estimates: Veronica
\item Bootstrapping of confidence intervals: Stephanie
\item Interpretation of results: Entire Group
\end{itemize}
